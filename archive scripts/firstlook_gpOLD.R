...
# In Tobii, everytime you create a AOI on a new movie/stimuli
...
firstlook_gpOLD <- function(subjects, neccessaryAOI, aoi1, aoi2) {
  ...
  for (s in 1:length(subjids)) {
    ...
    for (i in starti:(endi-1)) {
      ...
      # print(neccessaryAOI[i])
      if (neccessaryAOI[i] == 1) {
