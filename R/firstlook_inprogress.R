...
# importing data
dataframe <- read.csv("testswitch.csv")

firstlook_inprogress <- function(dataframe, subjectID, handAOI, AOI1, AOI2) {
  # First the variables need to be changed
  subjects <- dataframe["subjectID"]
  trialtype <- dataframe["trialname"]
  handAOI <- dataframe["hand"]
  aoi1 <- dataframe["col1"]
  aoi2 <- dataframe["col2"]
  # make new dataframe
  newdata <- cbind(subjects, trialtype, handAOI, aoi1, aoi2)
  newdata <- data.frame(newdata)
  colnames(newdata) <- c("subjects", "trialtype", "neccessaryAOI", "aoi1", "aoi2")
  print("newdata")
  print(newdata)
