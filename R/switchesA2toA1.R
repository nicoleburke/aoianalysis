...
#     of contingent switches between aoi2 to aoi1.
# This is the same function as A1toA2 - just going the other
#     way.
...
switchesA2toA1_inprogress<- function(data_by_trial, subjectID, AOI1, AOI2) {
  ...
  numA2toA1 <- matrix(nrow = length(subjids), ncol = 1)
  row.names(numA2toA1) <- subjids
  colnames(numA2toA1) <- c("NumSwtichesA2toA1")
  ...
  for (s in 1:length(subjids)) {
    ...
    for (i in starti:endi) {
      ...
      if (newdata$aoi2[i] == 1) {
        ...
        if (newdata$aoi2[i+1] == 0) {
          # Finally, check if there is a contingent hit in A2
          if (newdata$aoi1[i+1] == 1) {
            numswitchA2toA1 <- numswitchA2toA1 + 1
          }
        }
      }
      # Add number of swtiches to dataframe per subject
      numA2toA1[s, 1] <- numswitchA2toA1
    }
  }
  return(numA2toA1)
}
