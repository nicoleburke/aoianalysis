...
switchestotal <- function(data_by_trial, subjectID, AOI1, AOI2) {
  ...
  numswitch <- matrix(nrow = length(subjids), ncol = 3)
  row.names(numswitch) <- subjids
  colnames(numswitch) <- c("A1toA2", "A2toA1", "TotalSwitch")
  ...
  for (s in 1:length(subjids)) {
    ...
    for (i in starti:endi) {
      ...
      numswitch[s, 1] <- numswitchA1toA2
    }
    ### Calculate A2 to A1
    numswitchA2toA1 <- 0
    for (i in starti:endi) {
      # Look for a hit in A2
      if (newdata$aoi2[i] == 1) {
        # Check that there is not a look in the following line for A2
